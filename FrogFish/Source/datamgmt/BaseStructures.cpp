...
void assign_structures_to_bases(
    ...
    BWAPI::Race enemy_race = Broodwar->enemy()->getRace();
    if (enemy_race == BWAPI::Races::Zerg) {
        enemy_assign_structures_zerg_morph(
            the_map, base_storage, changed_type_enemy_units, enemy_bases);
    }
    else {
        enemy_assign_structures(the_map, base_storage, changed_type_enemy_units, enemy_bases);
    }
    if (enemy_race == BWAPI::Races::Terran || enemy_race == BWAPI::Races::Zerg) {
