...
const BWAPI::UnitType MakeQueue::types[8] = {
...
void MakeQueue::take_proportion_order(
...
void MakeQueue::take_exact_order(const std::vector<UnitType> &unit_types) {
    queue.clear();
...
const std::deque<BWAPI::UnitType> &MakeQueue::get_queue() {
    return queue;
}
...
BWAPI::UnitType MakeQueue::front() {
    if (queue_size > 0) {
        return queue.front();
    }
...
void MakeQueue::push_overlords_front(int count) {
    for (int i = 0; i < count; i++) {
        queue.push_front(BWAPI::UnitTypes::Zerg_Overlord);
    }
}

bool MakeQueue::order_filled() {
    return queue_size == 0;
}
